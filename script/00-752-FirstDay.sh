...
if [ -d /home/repositorio/FREQUENTES/752 ]
  then
    for OVA in $(ls -1 /home/repositorio/FREQUENTES/752/*752*.ova)
	    do
...
#------------------------------------------------------------------------------#
    ...
    for VM in $(vboxmanage list vms | awk '{ print $1}' | sed s/\"//g )
      do
        ...
        vboxmanage modifyvm ${VM} --groups "/752"
	    done
#------------------------------------------------------------------------------#
